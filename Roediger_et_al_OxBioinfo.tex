...
interest in amplification based techniques, there are only few tools 
...
chain reactions (qPCR) and quantitative isothermal amplification (qIA). The 
package takes advantage of \textbf{R}’s \emph{S4} object model and offers an 
extensible environment. Functions to normalize amplification curves, baseline 
amplification curves, to detect the start and end of an amplification reaction, 
a powerful wrapper for smoothers are part of the software. Moreover, 
$\emph{chipPCR}$ employs a 5-point stencil for derivative interpolation, which 
has not been described for \textbf{R} to the best of our knowledge. Graphical 
user interfaces for numerous fun$\emph{chipPCR}$ is an \textbf{R} package for 
pre-processing and quality analysis of amplification curve data from 
conventional quantitative polymerase chain reactions (qPCR) and quantitative 
isothermal amplification (qIA). The package takes advantage of \textbf{R}’s 
\emph{S4} object model and offers an extensible environment. Functions to 
normalize amplification curves, baseline amplification curves, to detect the 
start and end of an amplification reaction, a powerful wrapper for smoothers 
are part of the software. Moreover, $\emph{chipPCR}$ employs a 5-point stencil 
for derivative interpolation, which has not been described for \textbf{R} to the 
best of our knowledge. Graphical user interfaces for numerous functions were 
implemented as desktop or server applications. The package contains several data 
...
detection systems, such as hydrolysis probes and intercalating dyes. Functions 
were implemented as desktop or server applications. The package contains several 
data sets, which were generated by helicase dependent amplification (HDA) or 
polymerase chain reaction (PCR) under various temperature conditions and 
detection systems, such as hydrolysis probes and intercalating dyes.

\parttitle{Conclusion}
We have developed $\emph{shiny}$, which is a versatile software tailored for 
the pre-processing of amplification curve data. Its utility is elaborated on 
both real and simulated data sets. The structure of the packages is open for 
...
Contact: stefan.roediger@hs-lausitz.de
...
gradient (e.g., microfluidics) emerged \cite{rodiger_nucleic_2014, chou_rapid_2011}. Both 
amplification methods are used in different real-time monitoring technologies, 
such as our previously reported VideoScan technology, microfluidic systems, 
point-of-care devices, microbead-chip technologies and commercial real-time 
thermo cyclers \cite{rodiger_nucleic_2014,chang_2012, roediger_highly_2013, 
...
for gene expression analysis), and (5) data export, visualization and report generation.

  \textbf{R} is one of the most used tools in bioinformatics and is known to be 
an early adopter of emerging technologies such as digital PCR and  NanoString 
...
qPCR systems. Sophisticated \textbf{R} packages for the steps 1. and 3.--5. are 
available from Bioconductor and CRAN 
\cite{Dvinge_2009,perkins_2012,blagodatskikh_2014,heckmann_2011,pan_2012, 
Zhang_2010,mccall_2014,roediger_RJ_2013,gehlenborg_2013,huntley_2013,zhang_2013}
. However, there is no \textbf{R} package for pre-processing and quality 
analysis of amplification curve data. Novel developed qPCR and qIA technologies 
initially depend on tools to pre-process the raw data. Pre-processing 
specifically addresses raw data inspection, steps to transform raw data in a 
compatible format for successive analysis steps (e.g., smoothing, imputation of 
missing values), data reduction (e.g., removal of invalid sets) and data quality 
management. The data quality of experimental instruments is often not ready for 
end-user analysis and presentation. It is important to use as many raw data as 
possible. Pre-processing algorithms remove stochastic errors and artefacts 
(e.g., noise, photo-bleaching effects, degassing effects, different signal 
levels) illustrated in Figure~\ref{figure:problems}. Misinterpretations are more 
likely if non or manual (``arbitrary'') corrections are performed. A manual 
alteration is in contradiction to reproducible research. 
...
tools. Moreover, it is desirable set up workflows in an open environment, which 
...
Selected functionality is used in the $\emph{dpcR}$ 
\cite{burdukiewicz_2014} and $\emph{RDML}$ \cite{blagodatskikh_2014} packages.
...
blagodatskikh_2014}. The results of $\emph{chipPCR}$ functions are usually S4 
...
is often useful to smooth or filter the data. Therefore we developed the \textsl{smoother} function, which is a wrapper for 
smoother functions and filters commonly used to process amplification curve 
data (supplement Section~\ref{sec:smoothing_filterring}). \textsl{smoother} inherited traits (supplement Table~\ref{table:tab1})  of the 
...
sampling. The function \textsl{smoother} and \textsl{CPP} (inherited from 
