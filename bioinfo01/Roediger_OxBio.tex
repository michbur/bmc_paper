...
package. $\emph{chipPCR}$ core functionalities are integrated in GUIs 
...
structure allows users to perform flexible data analysis adjusted to their 
needs. Users can do estimations by hand. For example for Cq (SDM) estimation, 
solely the $\emph{chipPCR}$ functions \textsl{inder} and \textsl{smoother} are 
needed. \textsl{smoother} will be a method of smoothing in \textsl{inder}, and 
by putting data in the \textsl{bg} object with summary method 
\textsl{summary-der}, the user obtains the Cq. Through GUI's it should be easy 
for users without any \textbf{R} experience omitting a big limitation of qPCR 
and qIA \textbf{R} packages.
