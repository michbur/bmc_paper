...
package takes advantage of \textbf{R}’s \emph{S4} object model. Functions to 
normalize amplification curves, baseline amplification curves, to detect the 
start and end of an amplification reaction, a powerful wrapper for smoothers are 
part of the software. Moreover, $\emph{chipPCR}$ employs a 5-point stencil for 
derivative interpolation, which has not been described for \textbf{R} to the 
...
amplification efficiency (AE) as described elsewhere 
\cite{ruijter_2013,tellinghuisen_2014}. Fundamental steps of amplification curve 
...
secondary parameters (e.g., Delta-Delta-Ct for gene expression analysis), and 
...
validate experiments from microarrays, bead arrays and related technologies. qIA 
has an important role in point of care diagnostics.
...
for statistical computing and graphics. The supplemental material, available as the 
